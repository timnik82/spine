...
`stopwatch-end-chime.mp3` and `stopwatch-end-bell.mp3` are candidates for the end-of-countdown cue, kept for comparison and **not wired up yet** — nothing decodes them and the last three ticks do not beep. Pick one before implementing the cue and delete the other; both currently ship in the build because everything in `public/` is copied verbatim. They were generated with `npm run sfx -- "short friendly rising two-tone chime, completion sound, clean" --duration 1.2` and `npm run sfx -- "single warm soft bell ding, short decay, gentle, clean" --duration 1.5`.
